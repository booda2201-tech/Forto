...
export class WorkerPageComponent implements OnInit {
  ...
  startExecution(task: any) {
    this.api.startBookingItem(task.bookingItemId, { employeeId: this.employeeId }).subscribe({
      next: () => {
  ...
  finishExecution(task: any) {
    this.api.completeBookingItem(task.bookingItemId, { employeeId: this.employeeId }).subscribe({
      next: () => {
  ...
  saveMaterials() {
    ...
    const payload = {
      ...
      materials: this.selectedMaterialRows
        ...
        .map(m => ({ materialId: Number(m.materialId), actualQty: Number(m.actualQty) }))
    };

    this.api.updateBookingItemMaterials(this.selectedTask.bookingItemId, payload).subscribe({
      next: () => alert('تم حفظ المواد'),
      error: (err) => {
        console.error(err);
  ...
  statusText(status: number) {
    return status === 1 ? 'قيد الانتظار'
      : status === 2 ? 'قيد التنفيذ'
      : 'مكتمل';
  }
